...
// Chrome Native Messaging host for Iris.
...
function handleGetConfig(id) {
  const irisConfigPath = path.join(os.homedir(), ".iris", "config.json");
  const legacyConfigPath = path.join(os.homedir(), ".opencode-browser", "config.json");
  let config = {};
  try {
    if (fs.existsSync(irisConfigPath)) {
      config = JSON.parse(fs.readFileSync(irisConfigPath, "utf8"));
  ...
  writeNativeMessage({ type: "config_response", id, config });
}
...
(async () => {
  ...
  process.stdin.on("data", (chunk) =>
    onStdinData(chunk, (message) => {
      // Handle get_config locally — do not forward to broker.
      if (message && (message.op === "get_config" || message.type === "get_config")) {
        handleGetConfig(message.id);
        return;
      }
